/**
 * GESTÃO DE MEMBROS - IPB GURIRI
 * Versão V7 - Estrutura Centralizada (entidades/ipb_guriri/dados)
 */

// === CONFIGURAÇÃO DE CAMINHOS DO BANCO (ORGANIZAÇÃO) ===
const ENTIDADE = 'ipb_guriri';
const CAMINHO_BASE = `entidades/${ENTIDADE}/dados`;

const REF_MEMBROS    = `${CAMINHO_BASE}/membros`;
const REF_SOCIEDADES = `${CAMINHO_BASE}/sociedades`; // Antigo 'tags'
const REF_SALAS      = `${CAMINHO_BASE}/salas`;
const REF_CARGOS     = `${CAMINHO_BASE}/cargos`;
...
function carregarConfiguracoesTema() {
    ...
    database.ref(`${CAMINHO_BASE}/configuracoes/temaColor`).on('value', (snapshot) => {
...
function carregarDadosAuxiliares() {
    ...
    database.ref(REF_SOCIEDADES).on('value', snap => {
    ...
    // 2. SALAS
    database.ref(REF_SALAS).on('value', snap => {
        cacheSalas = snap.val() || {};
    ...
    // 3. CARGOS
    database.ref(REF_CARGOS).on('value', snap => {
        cacheCargos = [];
        if(snap.exists()) {
            snap.forEach(child => {
                const val = child.val();
...
function listarMembros() {
    database.ref(REF_MEMBROS).on('value', snap => {
        membrosLocais = [];
        if (snap.exists()) {
            snap.forEach(child => {
                const val = child.val();
...
function renderizarTabelaMembros(lista) {
    ...
    lista.forEach(m => {
        // Correção de exibição da Sociedade
        const idSociedade = m.sociedadeId;
        // Se não achar a cor exata, usa cinza para não ficar invisível
...
// 1. SOCIEDADES (Caminho: dados/sociedades)
...
function salvarTagSociedade() {
    ...
    const db = firebase.database().ref(REF_SOCIEDADES);
...
function salvarTagSala() {
    ...
    const db = firebase.database().ref(REF_SALAS);
...
function salvarTagCargo() {
    ...
    const db = firebase.database().ref(REF_CARGOS);
...
function salvarNovoCargoRapido(nomeOriginal) {
    ...
    database.ref(REF_CARGOS).push({ nome: nome }).then(() => {
...
document.getElementById('btnConfirmarExclusao').onclick = function() {
    ...
    if(tipoParaExcluir === 'membro') {
        refPath = `${REF_MEMBROS}/${itemParaExcluir}`;
        promise = database.ref(refPath).remove();
    } 
    else if(tipoParaExcluir === 'sociedade') {
        promise = database.ref(`${REF_SOCIEDADES}/${itemParaExcluir}`).remove()
            .then(() => atualizarVinculoMembros('sociedadeId', itemParaExcluir, null));
    }
    else if(tipoParaExcluir === 'sala') {
        promise = database.ref(`${REF_SALAS}/${itemParaExcluir}`).remove()
        ...
        promise = database.ref(`${REF_CARGOS}/${itemParaExcluir}`).remove().then(() => {
...
function atualizarVinculoMembros(campo, velho, novo) {
    membrosLocais.forEach(m => {
        ...
        if(mudou) database.ref(`${REF_MEMBROS}/${m.id}`).update(m);
    });
}
...
function salvarMembro() {
    ...
    if(idMembroEdicao) database.ref(`${REF_MEMBROS}/${idMembroEdicao}`).update(d).then(() => { mostrarToast("Salvo!"); fecharModalMembro(); });
    else database.ref(REF_MEMBROS).push(d).then(() => { mostrarToast("Criado!"); fecharModalMembro(); });
}
